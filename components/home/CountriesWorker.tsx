...
const COUNTRIES: CountryRow[] = [
  { id: "uae", flag: "🇦🇪", name: "United Arab Emirates", code: "DXB", region: "Middle East & Gulf", jobs: 245 },
  { id: "ksa", flag: "🇸🇦", name: "Saudi Arabia", code: "RUH", region: "Middle East & Gulf", jobs: 180 },
  { id: "qat", flag: "🇶🇦", name: "Qatar", code: "DOH", region: "Middle East & Gulf", jobs: 95 },
  { id: "kwt", flag: "🇰🇼", name: "Kuwait", code: "KWI", region: "Middle East & Gulf", jobs: 75 },
  { id: "omn", flag: "🇴🇲", name: "Oman", code: "MCT", region: "Middle East & Gulf", jobs: 60 },
  { id: "mys", flag: "🇲🇾", name: "Malaysia", code: "KUL", region: "Asia Pacific", jobs: 55 },
  { id: "bhr", flag: "🇧🇭", name: "Bahrain", code: "BAH", region: "Middle East & Gulf", jobs: 40 },
  { id: "sgp", flag: "🇸🇬", name: "Singapore", code: "SIN", region: "Asia Pacific", jobs: 30 },
  { id: "rou", flag: "🇷🇴", name: "Romania", code: "OTP", region: "Europe", jobs: 25 },
  { id: "hrv", flag: "🇭🇷", name: "Croatia", code: "ZAG", region: "Europe", jobs: 20 },
];

const TOTAL_JOBS = COUNTRIES.reduce((sum, c) => sum + c.jobs, 0);
const TOTAL_REGIONS = new Set(COUNTRIES.map((c) => c.region)).size;
const GULF_COUNT = COUNTRIES.filter((c) => c.region === "Middle East & Gulf").length;
...
function getDemandBadge(jobs: number): { label: string; className: string } {
  if (jobs >= 150) {
    return { label: "Hiring fast", className: "bg-[var(--cw-orange)] text-white" };
  }
  if (jobs >= 50) {
    return { label: "Steady demand", className: "bg-[var(--cw-orange-light)] text-[var(--cw-orange-deep)]" };
  }
  return { label: "Selective hiring", className: "bg-stone-100 text-stone-500" };
}
...
const SECTOR_GROUPS: SectorGroup[] = [
  { id: "g-uae", flags: "🇦🇪", destination: "United Arab Emirates", sectors: ["Construction", "Hospitality", "Logistics", "Retail", "Healthcare"] },
  { id: "g-ksa", flags: "🇸🇦", destination: "Saudi Arabia", sectors: ["Construction", "Engineering", "Maintenance", "Healthcare"] },
  { id: "g-qat", flags: "🇶🇦", destination: "Qatar", sectors: ["Infrastructure", "Hospitality", "Facility Management"] },
  { id: "g-kwt-omn", flags: "🇰🇼🇴🇲", destination: "Kuwait & Oman", sectors: ["Oil & Gas Support", "Construction", "Technical Trades"] },
  { id: "g-mys-sgp", flags: "🇲🇾🇸🇬", destination: "Malaysia & Singapore", sectors: ["Manufacturing", "Electronics", "Services"] },
  { id: "g-eu", flags: "🇷🇴🇭🇷", destination: "Romania & Croatia", sectors: ["Skilled Trades", "Manufacturing", "Warehousing", "Hospitality"] },
];
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 pt-20 pb-16 sm:pt-28 sm:pb-20 lg:px-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-[var(--cw-line)] bg-[var(--cw-bg-warm)] px-4 py-1.5">
            <Plane className="h-3.5 w-3.5 text-[var(--cw-orange)]" aria-hidden="true" />
          ...
          <p className="mt-5 max-w-xl text-lg leading-relaxed text-[var(--cw-ink-soft)]">
            Many job seekers start with a destination in mind. Explore opportunities
            by country and see exactly where we&apos;re hiring right now.
          </p>
          ...
          <div className="mt-14 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-[var(--cw-line)] bg-[var(--cw-line)] sm:grid-cols-4">
            {STATS.map((stat) => (
              <div key={stat.label} className="bg-white px-6 py-6 text-center sm:text-left">
    ...
    <table className="w-full border-collapse text-left">
      ...
      <div className="mx-auto max-w-7xl px-6 pb-20 lg:px-8">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-[var(--cw-ink-soft)]">
            Whether it&apos;s construction in the Gulf, hospitality in the Middle East,
            manufacturing in Asia, or skilled trades in Europe, we connect workers
            with verified employers across every country we serve.
          </p>
        </div>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {SECTOR_GROUPS.map((group) => (
            ...
            >
              ...
              <div
                className="relative flex w-16 shrink-0 flex-col items-center justify-center border-l-2 border-dashed border-[var(--cw-line)] bg-white/70"
              >
                <Link
          ...
          {WHY_REASONS.map(({ id, Icon, title, description }) => (
            <div key={id} className="flex flex-col gap-3 bg-white p-7">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--cw-orange-light)]">
                <Icon className="h-5 w-5 text-[var(--cw-orange-deep)]" aria-hidden="true" />
              </div>
        ...
        <div className="cw-scallop-top mt-2 overflow-hidden rounded-b-3xl bg-gradient-to-br from-[var(--cw-orange)] to-[var(--cw-orange-deep)] px-8 py-14 sm:px-14 sm:py-16">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-base leading-relaxed text-white/90">
              Browse current openings by country and apply for positions that match
              your experience and career goals. Our recruitment team can guide you
              through the application, documentation, and deployment process from
              start to finish.
            </p>
